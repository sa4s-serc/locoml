...
export default memo(({id, data, isConnectable}) => {
    const [MTModels, setMTModels] = useState({});
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_GET_TRAINED_MODELS_URL}/MT`);
                console.log("Received MT models: ", response.data);
                ...
                parsedModels.forEach((model) => {
                    MTModelMap[model.model_id] = model;
                });
                setMTModels(MTModelMap);
                setIsLoading(false);
            } catch (error) {
                console.log(error);
            }
    ...
    const handleModelChange = (value) => {
        ...
        const selectedModel = MTModels[value];
    ...
    return (<>
        ...
        <div className="switchNode" onClick={() => setIsModalOpen(true)}>
            ...
            <div className="switchLabel">MT</div>
            <Button
                ...
                <AccordionDetails>
                    <FormControl fullWidth>
                        <Select
                            ...
                            options={Object.keys(MTModels).map((model_id) => ({
                                value: model_id, label: MTModels[model_id].model_name
                            }))}
                            disabled={isLoading}
